...
func TestBuildNetworkFilterPolicySnapshotBuildsGuestRulesFromActiveExecutions(t *testing.T) {
	originalLookup := lookupNetworkFilterHostIPs
	lookupNetworkFilterHostIPs = func(_ context.Context, host string) ([]netip.Addr, error) {
		switch host {
		case "github.com":
	...
	defer func() { lookupNetworkFilterHostIPs = originalLookup }()
	...
	snapshot := svc.buildNetworkFilterPolicySnapshot("/tmp/cleanroom-darwin-vz")
	if len(snapshot.GuestRules) != 1 {
		t.Fatalf("expected 1 guest rule, got %#v", snapshot.GuestRules)
	}
	guestRule := snapshot.GuestRules[0]
	if got, want := guestRule.GuestIP, "10.233.0.2"; got != want {
		t.Fatalf("guest_ip = %q, want %q", got, want)
	}
	if got, want := guestRule.DefaultAction, networkFilterActionDeny; got != want {
		t.Fatalf("guest default action = %q, want %q", got, want)
	}
	if !guestRule.AllowDNS {
		t.Fatal("expected deny-mode guest rule to allow DNS bootstrap")
	}
	wantAllow := []networkFilterPolicyAllowRule{
		{Host: "github.com", Ports: []int{443}, RemoteIPs: []string{"140.82.112.4"}},
	}
	if !reflect.DeepEqual(guestRule.Allow, wantAllow) {
		t.Fatalf("guest allow rules mismatch:\n got=%v\nwant=%v", guestRule.Allow, wantAllow)
	}
}
...
func TestSyncNetworkFilterPolicyPublishesToDaemon(t *testing.T) {
	t.Setenv(networkFilterTargetProcessEnv, "/tmp/cleanroom-darwin-vz")

	store, err := networkfilterstate.NewStore(t.TempDir())
	if err != nil {
		t.Fatalf("NewStore returned error: %v", err)
	}
	server := networkfilterstate.NewServer(store)
	...
	t.Setenv(networkFilterDaemonURLEnv, "http://"+listener.Addr().String())
	...
	svc.SyncNetworkFilterPolicy()

	snapshot, ok := store.GetPolicy()
	if !ok {
		t.Fatal("expected daemon to have a published policy snapshot")
	}
	if got, want := snapshot.DefaultAction, networkFilterActionDeny; got != want {
		t.Fatalf("default action = %q, want %q", got, want)
	}
