...
CLEANROOM_RELEASE_REPO='$CLEANROOM_RELEASE_REPO'
...
export CLEANROOM_RELEASE_REPO="$CLEANROOM_RELEASE_REPO"
...
install_cleanroom_release() {
  ...
  if [ "$release_version" = "latest" ]; then
    retry 5 5 env \
      ...
      CLEANROOM_PREFIX="$CLEANROOM_INSTALL_PREFIX" \
...
if [ -z "$CLEANROOM_INSTALL_PREFIX" ] && [ -n "${CLEANROOM_BINARY_INSTALL_DIR:-}" ]; then
  case "$CLEANROOM_BINARY_INSTALL_DIR" in
    */bin) CLEANROOM_INSTALL_PREFIX="${CLEANROOM_BINARY_INSTALL_DIR%/bin}" ;;
    *) die "CLEANROOM_BINARY_INSTALL_DIR must end with /bin when CLEANROOM_INSTALL_PREFIX is not set" ;;
  esac
fi
CLEANROOM_INSTALL_PREFIX="${CLEANROOM_INSTALL_PREFIX:-/usr/local}"
CLEANROOM_BINARY_INSTALL_DIR="${CLEANROOM_BINARY_INSTALL_DIR:-$CLEANROOM_INSTALL_PREFIX/bin}"
...
HELPER_INSTALL_PATH="${CLEANROOM_HELPER_INSTALL_PATH:-$CLEANROOM_INSTALL_PREFIX/libexec/cleanroom/cleanroom-root-helper}"
...
RELEASE_REPO="${CLEANROOM_RELEASE_REPO:-}"
if [ -z "$RELEASE_REPO" ]; then
  RELEASE_REPO="$(github_repo_slug_from_url "$BOOTSTRAP_REPO_URL" 2>/dev/null || true)"
fi
[ -n "$RELEASE_REPO" ] || die "set CLEANROOM_RELEASE_REPO when CLEANROOM_BOOTSTRAP_REPO_URL is not a supported GitHub remote"
