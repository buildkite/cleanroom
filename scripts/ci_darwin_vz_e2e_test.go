...
func TestCiDarwinVZVMNetE2EUsesBuildkiteSecretsAndVMNetEntitlements(t *testing.T) {
	...
	for _, needle := range []string{
		...
		"AppleWWDRCAG3.cer",
		`run_with_macos_user_home security add-certificates -k "$temp_keychain_path" "$wwdr_path"`,
		"warning: unable to add WWDR intermediate to temp keychain; relying on existing system/login keychains",
