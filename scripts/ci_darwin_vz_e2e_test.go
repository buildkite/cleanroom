...
func TestCiDarwinVZVMNetE2EUsesBuildkiteSecretsAndVMNetEntitlements(t *testing.T) {
	...
	for _, needle := range []string{
		...
		`staged_helper="$REPO_ROOT/$(cleanroom_stage_libexec_path cleanroom-darwin-vz.app)"`,
		`legacy_helper="$REPO_ROOT/dist/cleanroom-darwin-vz.app"`,
		`if [[ -d "$staged_helper" ]]; then`,
		...
		`printf '%s\n' "$legacy_helper"`,
		`bash -x scripts/build-darwin-vz-helper.sh dist/cleanroom-darwin-vz.app`,
