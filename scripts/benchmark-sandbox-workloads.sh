...
for _ in $(seq 1 "$iterations"); do
  iops_script="set -eu; f=/tmp/cleanroom-iops.bin; bs=${iops_block_size}; c=${iops_ops}; ws=\$(awk '{print \$1}' /proc/uptime); dd if=/dev/zero of=\"\$f\" bs=\"\$bs\" count=\"\$c\" conv=fdatasync >/dev/null 2>&1; we=\$(awk '{print \$1}' /proc/uptime); rs=\$(awk '{print \$1}' /proc/uptime); dd if=\"\$f\" of=/dev/null bs=\"\$bs\" count=\"\$c\" >/dev/null 2>&1; re=\$(awk '{print \$1}' /proc/uptime); rm -f \"\$f\"; awk -v ws=\"\$ws\" -v we=\"\$we\" -v rs=\"\$rs\" -v re=\"\$re\" -v c=\"\$c\" -v bs=\"\$bs\" 'BEGIN { wt=we-ws; rt=re-rs; wi=(wt<=0)?0:(c/wt); ri=(rt<=0)?0:(c/rt); wmb=(wt<=0)?0:((bs*c/1048576)/wt); rmb=(rt<=0)?0:((bs*c/1048576)/rt); printf \"%.6f %.2f %.2f %.6f %.2f %.2f\\n\", wt, wi, wmb, rt, ri, rmb }'"
